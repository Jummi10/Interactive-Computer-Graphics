...
void render()
{
	...
	sc.model = translate(mat4(1.0f), vec3(robot.opt[0], 0.0, robot.opt[1]));	//base의 축 결정
	sc.model = rotate(sc.model, (float)radians(robot.opt[2]), vec3(0.0, 1.0, 0.0));	//hierarchical relationship
	sc.mvs.push_back(sc.model);	//bt*br
	// base instance transformation
	sc.model = translate(sc.model, vec3(0.0, robot.BASE_HEIGHT/2.0, 0.0));	//TRS
	sc.model = scale(sc.model, vec3(1.0, robot.BASE_HEIGHT, 1.0));
	glUniformMatrix4fv(sc.m_loc, 1, GL_FALSE, value_ptr(sc.model));
	glUniform3f(sc.color_loc, 0.3, 0.2, 0.9);
	sc.cube->render();
	sc.model = sc.mvs.back(); sc.mvs.pop_back();	//저장해놓은거 다시 가져와=>sc.model=bt*br
	...
	sc.mvs.push_back(sc.model);	//bt*br*Lt*Lrx*Lrz
	...
	sc.model = rotate(sc.model, (float)radians(robot.opt[6]), vec3(0.0, 0.0, 1.0));	//값을 얼마나 주느냐에 따라서 z축
	sc.model = rotate(sc.model, (float)radians(robot.opt[5]), vec3(1.0, 0.0, 0.0));	//x축
	sc.mvs.push_back(sc.model);	//스택에 더 그릴까봐 넣어줌
