...
void startup()
{
	...
	sc.view = LookAt(vec3(0.0f,4.0f,14.0f), vec3(0.0f,0.75f,0.0f), vec3(0.0f,1.0f,0.0f));
	...
	glUniform1f(glGetUniformLocation(sc.rendering_program,"Spot.exponent"),3.0f);	//값 키우면 어두워진다.
	glUniform1f(glGetUniformLocation(sc.rendering_program,"Spot.cutoff"),30.0f);	//30도, 30도해서 60도
}
...
void render()
{
	...
	// Lighting
	vec4 lightPos = vec4(15.0f * cos(sc.angle), 15.0f, 15.0f * sin(sc.angle), 1.0f);	//빛 회전
